...
import {
  ...
  createMiembro,
  updateMiembro,
} from "../services/api"; // Removed unused deleteMiembro
...
export const MiembrosGrupo = ({
  ...
  const [editandoMiembro, setEditandoMiembro] = useState<MiembroEdicion | null>(
    null
  );
  ...
  const getNombreCompleto = (usuario: Usuario) => {
    return `${usuario.nombre || ""} ${usuario.primer_apellido || ""}${
  ...
  }, [grupoId, cargarUsuarios, cargarMiembros]);
  ...
  const handleEliminarMiembro = async (miembroId: number) => {
    if (
      !window.confirm(
        "¿Está seguro de dar de baja a este miembro del grupo? Se establecerá la fecha de fin al día actual."
      )
    )
      return;
      ...
      <Box>
          ...
          <Autocomplete
            options={usuarios.filter((usuario) => {
              ...
              return !miembros.some(
                (m) => m.persona_id === usuario.id && !!m.activo === true
              );
            })}
        ...
        <Box>
          <FormControlLabel
            ...
            label="Mostrar miembros inactivos"
          />
        </Box>
        ...
        {cargando ? (
          ...
          <Table size="small">
            ...
            <TableBody>
              {miembros
                .filter((miembro) => showInactive || miembro.activo) // Only show active members in the table
                .map((miembro) => (
                  <TableRow key={miembro.id}>
                    <TableCell>
                      {miembro.persona?.nombre}{" "}
                      {miembro.persona?.primer_apellido}{" "}
                      {miembro.persona?.segundo_apellido || ""}
                    </TableCell>
                    <TableCell>
                      {miembro.fecha_inicio
                        ? new Date(miembro.fecha_inicio).toLocaleDateString()
                        : "-"}
                    </TableCell>
                    <TableCell>
                      {miembro.fecha_fin
                        ? new Date(miembro.fecha_fin).toLocaleDateString()
                        : "-"}
                    </TableCell>
                    <TableCell>
                      {miembro.fecha_fin ? (
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={guardando}
            startIcon={
              guardando ? <CircularProgress size={20} /> : <SaveIcon />
            }
          >
